...
public class Calculation {
    ...
    public Calculation(double creditSum, int payoutDuration, double percentRate, LocalDate startDate, ExtraPaymentsMap extraPaymentsMap) {
        ...
        Date start = new Date();
        setPureGraph();
        System.out.println( "Время выполнения: " + (new Date().getTime() - start.getTime()) );

    }

    public Calculation(double creditSum, double monthPay, double percentRate, LocalDate startDate, ExtraPaymentsMap extraPaymentsMap) {
        this.creditSum = creditSum;
        this.percentRate = percentRate;
        this.startDate = startDate;
    ...
    public Calculation(int payoutDuration, double monthPay, double percentRate, LocalDate startDate, ExtraPaymentsMap extraPaymentsMap) {
        this.payoutDuration = payoutDuration;
        this.percentRate = percentRate;
        this.startDate = startDate;
        this.monthPay = monthPay;
    ...
    public Calculation(double creditSum ,int payoutDuration , double percentRate, LocalDate startDate, CreditType creditType, ExtraPaymentsMap extraPaymentsMap) {
        this.payoutDuration = payoutDuration;
        this.creditSum = creditSum;
        this.percentRate = percentRate;
        this.startDate = startDate;
    ...
    private void setMonthPay(double creditSum, boolean isAfterExtraPay) {
        ...
        } while (!checked(monthPayPrev , monthPayLocal));
        this.monthPay = monthPayLocal;
    }
    private boolean checked(double monthPayPrev, double monthPayLocal) {
    ...
    private void setPayoutDuration() {
        ...
        LocalDate date = new LocalDate(lastPayDay);
        LocalDate secondDate = new LocalDate(nowPayDay);
        int payoutDuration = 0;

        while (creditSum != 0) {
            if (creditSum > monthPay) {
                creditSum = creditSum - (monthPay - findPercents(date, secondDate , creditSum));
                date = new LocalDate(secondDate);
                secondDate = secondDate.plusMonths(1);
                payoutDuration++;
            } else {
                creditSum = 0;
                payoutDuration++;
    ...
    private void addPurePayment() {
        ...
        if (beforePayCreditSum > basicPay && !checked(beforePayCreditSum, basicPay)) {
...
//---------------------------------------------------------------------------------------------------------
    ...
    private synchronized void addToMap(double payDayCreditSum, double monthPay, double percents, double basicPay) {
                    ...
                    temp.put(GraphColumn.BASIC_PAYMENT,  basicPay < 0 ? "+" + String.format(format, Math.abs(basicPay)) : String.format(format, basicPay));
...
//---------------------------------------------------------------------------------------------------------
    ...
    private BigDecimal getDaysOfMonthInYear(LocalDate lastPayDay, LocalDate nowPayDay) {
        ...
        if (!lastPayDay.year().equals(nowPayDay.year())) {
            ...
            daysOfMonthInYear = BigDecimal.valueOf(Days.daysBetween(lastPayDay, nowPayDay).getDays()).divide(BigDecimal.valueOf(lastPayDay.dayOfYear().getMaximumValue()), 15, RoundingMode.DOWN);
    }
        return daysOfMonthInYear;
}
}
